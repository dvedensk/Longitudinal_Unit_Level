...
PLMBmcmc <- function(X, Psi, Y, sig2b=1000, iter=1000, burn=500, wgt=NULL){
 ...
 # pb <- txtProgressBar(min=0, max=iter, style=3)
  for(i in 1:iter){
    ...
    meanBeta <- t(X)%*%Diagonal(length(w),w)%*%(kappa/w - Psi_eta )
    U_beta <- base::chol(precBeta)
    tmp_norm <- rnorm(nrow(precBeta))
    #beta <- betaOut[i,]  <- as.numeric(rmvnorm(1, mean=precBeta%*%meanBeta, sigma=as.matrix(precBeta)))
    beta <- betaOut[i,]  <- backsolve(U_beta, backsolve(U_beta, meanBeta, transpose=TRUE) + tmp_norm)

    ## Sample random effects
    Einv <- (1/sig2E)*Diagonal(r)
    precEta <- t(Psi)%*%Diagonal(length(w),w)%*%Psi + Einv
    meanEta <- t(Psi)%*%Diagonal(length(w),w)%*%(kappa/w - X_beta )
    U_eta <- base::chol(precEta)
    tmp_norm <- rnorm(nrow(precEta))
    #eT <- as.numeric(rmvnorm(1, mean=precEta%*%meanEta, sigma=as.matrix(precEta)))
    eT <- backsolve(U_eta, backsolve(U_eta, meanEta, transpose=TRUE) + tmp_norm)
    #sum to zero
    ...
    w <- rpg.gamma(n, wgt, as.numeric(X_beta + Psi_eta))
