...
load(paste0(save_dir, "binary_simulation_results_postpreds.RData"))
load(paste0(save_dir, "direct_estimates.RData"))
load(paste0(save_dir, "UW_direct_estimates_binary.RData"))
...
Nsim <- nrow(postpreds)
n_weeks <- max(truth$WEEK)
#test_num <- 20

truth <- filter(truth)
#make tables
gibbs_dep_full <- map_df(postpreds[,"gibbs_dep_post_pred"],
                         bind_rows, 
                         .id="sim_num") %>% 
                  tibble() %>% 
...
gibbs_indep_full <- map_df(postpreds[,"gibbs_indep_post_pred"],
                           bind_rows, 
                           .id="sim_num") %>%
                  tibble() %>% 
                  mutate(sim_num=as.integer(gsub("result\\.","",sim_num)))
...
missing_rows <- direct_est_full %>%
    ...
    select(sim_num, AREA, WEEK, N, MEAN, VAR, CI_LOWER, CI_UPPER)
...
ggsave(paste0(plot_dir, "MSE_reduction.pdf"))

system2(command = "pdfcrop", 
        args    = c(paste0(plot_dir,"MSE_reduction.pdf"),
                    paste0(plot_dir,"MSE_reduction.pdf")))                    
